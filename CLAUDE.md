...
- `Implémenté` E12.56 [P] Unified model registry: all models (embedding, reranking, VLM, STT, LLM) in the llm registry with start/stop lifecycle. Specialized sections (embedding, reranking, parse, enrich) reference by name. Build/serve auto-start embedding service
