...
- `À faire` E10.32 [P] Config refactoring: LLM registry pattern. `llm:` top-level = list of available models (name, model, api_url, api_key). Each section (enrich, judge, parse) references models by name. List of models per section = dimensions for optimize. Replaces current flat `llm:` section. Depends on E10.30
