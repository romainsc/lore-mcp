...
- `Prêt` E12.28 [P] Multi-model image captioning: all configured caption models run sequentially on all images, one phase2-caption-{name}.md per model. OCR + Docling 26-class classifier cached. No hardcoded routing. Config: parse.caption_models list. See docs/studies/grooming-E12.28.md
- `À faire` E12.29 [E] Caption selection/fusion: configurable rules for selecting or fusing results from multiple caption models. Default: first non-empty. Options: longest, classification-based preference, LLM judge. Depends on E12.28
