...
- `À faire` E12.58 [P] Unified --allow-download flag: gate both model downloads (HuggingFace builtin) and source URL fetches (manifest url-only entries) behind explicit authorization. Default: no download without flag
- `Implémenté` E12.59 [P] Graceful shutdown on SIGINT/SIGTERM: catch signal, stop all running IS (stop_service), free VRAM, cleanup temp files, exit cleanly. Currently Ctrl+C leaves IS containers running and VRAM allocated: api_url in registry is base URL (http://host:port/v1). Code appends path per service type (/embeddings, /chat/completions, /audio/transcriptions). Health/models derived from base. Remove all hardcoded path appending inconsistencies: all models (embedding, reranking, VLM, STT, LLM) in the llm registry with start/stop lifecycle. Specialized sections (embedding, reranking, parse, enrich) reference by name. Build/serve auto-start embedding service
