...
- `Prêt` E12.34 [P] CUDA check in subprocess: torch.cuda.is_available() allocates 3 MiB VRAM that cannot be freed. Run CUDA diagnostic check in a short-lived subprocess to avoid polluting main process VRAM before IS start
