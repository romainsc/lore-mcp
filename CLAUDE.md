...
- `Prêt` E12.47 [P] Minimal function signatures: reduce preprocess_sources to 3 params, run_build to 4 params. All pipeline params resolved from LoreConfig. Migrate tests to LoreConfig fixture. See grooming-E12.47.md
- `À faire` E12.48 [E] Audio ingestion: deposit audio file (.mp3/.wav/.ogg/.m4a) → transcription (whisper or libre equivalent) → markdown as orig → standard preprocess pipeline applies. Needs study: libre STT models, integration point
- `À faire` E12.49 [E] Video ingestion: deposit video file (.mp4/.mkv/.webm) → audio transcription + pertinent frame capture (keyframes, slide changes, or transcription-guided) → markdown + images as orig → standard preprocess pipeline applies. Needs study: libre tools, frame extraction strategy
