...
- `À faire` E10.33 [E] Per-source chunking defaults study: identify patterns for auto-selecting chunk_size/overlap based on file content, format, size, density. Inform E6.10 per-source params
...
- `À faire` E12.45 [P] Standalone photo/infographic fallback: when Docling produces empty output on a standalone image (0 pictures, 0 texts in JSON), fall back to direct VLM captioning via API. Pexels photo produces 0 bytes currently. See audit-prep-full-tesseract-2026-09-22.md
- `À faire` E12.46 [P] Enrich meta labels in source language: translate Summary/Keywords labels to source language in enrich_meta output. Currently always English labels even on FR sources. See audit-prep-full-tesseract-2026-09-22.md
...
- `Prêt` E12.28 [P] Multi-model image captioning: all configured caption models run sequentially on all images, one phase2-caption-{name}.md per model. OCR cached. No hardcoded routing. Classification by each VLM (Docling classifier tested 0/5 correct on PPTX — rejected). Alt text as context not skip. caption_judge configurable. MVP = judge strategy. See docs/studies/grooming-E12.28.md
- E12.29 — absorbed into E12.28 Part C (judge selection as MVP of E12.28)
