...
- `À faire` E12.64 [P] Sub-source checkpoint for captioning: resume at the Nth image within a source, not from scratch. Detect already-captioned images in intermediate file. Critical for PPTX with 49+ images (2h captioning lost on interrupt)
- `À faire` E12.65 [P] Threaded blocking calls for immediate signal handling: wrap urllib/subprocess calls in threads with threading.Event for interruption. CPython signals are queued during C-level blocking calls — threading allows the main thread to catch SIGINT immediately
