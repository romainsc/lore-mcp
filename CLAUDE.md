...
- `À faire` E10.35 [P] Fix eval crash on empty FTS5 query: lore-mcp eval crashes when auto-generated questions produce empty strings after cleaning (headings-only or base64-only content). FTS5 rejects empty MATCH query. Guard: skip empty queries in eval, warn in report
