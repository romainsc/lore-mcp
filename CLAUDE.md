...
- `Implémenté` E12.36 [P] Enrichment in source language: "Respond in the same language as the content" added to all enrichment prompts
...
- `Implémenté` E12.38 [P] LLM post-OCR correction: correct_ocr() replaces regex _fix_ocr_artifacts. LLM corrects OCR artifacts on OCR sources only (has_ocr flag from phase1)
- `Implémenté` E12.39 [P] Preserve content during enrichment: "Preserve all original text. Add context alongside, do not replace or summarize" added to enrichment prompts
