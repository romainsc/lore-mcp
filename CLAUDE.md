...
- `À faire` E12.63 [P] Resumable pipeline: checkpoint state to disk after each phase/source, resume from last checkpoint on restart. Survive reboot during long runs (STT+captioning can take 5h+). Checkpoint file in prep_dir, read on startup, skip completed phases/sources
